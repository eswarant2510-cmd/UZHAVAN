...
function localPaymentGateway(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          if (pathname === "/api/create-razorpay-order") {
            if (req.method !== "POST") {
              res.statusCode = 405
            ...
            const { orderId, amount } = JSON.parse(rawBody)
            ...
            const amountVal = amount ? Number(amount) : 14800
            ...
            if (!KEY_ID || !KEY_SECRET) {
              ...
              return res.end(
                JSON.stringify({
                  ...
                  currency: "INR",
                  warning:
                    "Razorpay credentials not set - executing in Local Test simulation.",
                }),
              )
            }
            ...
            if (!rzpRes.ok) {
              ...
              res.setHeader("Content-Type", "application/json")
              return res.end(JSON.stringify(rzpBody))
            }
          ...
          if (pathname === "/api/verify-payment") {
            ...
            if (!KEY_SECRET) {
              if (
                ...
                razorpay_signature.startsWith("pay_")
              ) {
                verified = true
              }
            } else {
              ...
              const computedSig = crypto
                ...
                .update(text)
                .digest("hex")
              verified = computedSig === razorpay_signature
            }
            ...
            res.setHeader("Content-Type", "application/json")
            return res.end(JSON.stringify({ verified: true, status: "PAID" }))
          }
          ...
          if (pathname === "/api/razorpay-webhook") {
            ...
            if (WEBHOOK_SECRET) {
              ...
              if (computedSig !== headerSig) {
                res.statusCode = 400
                return res.end("Webhook Signature failed")
              }
            }

            const bodyObj = JSON.parse(rawBody)
            console.log("[Local Webhook Server] Event received:", bodyObj.event)

            res.statusCode = 200
            return res.end("Received")
          }

          if (pathname === "/api/settle-order") {
            if (req.method !== "POST") {
              res.statusCode = 405
              return res.end(JSON.stringify({ error: "Method not allowed" }))
            }
            ...
            if (!KEY_ID || !KEY_SECRET) {
              ...
              return res.end(
                JSON.stringify({
                  success: true,
                  warning:
                    "Settlement ready — provider configuration required. Running in Local Test simulation.",
                }),
              )
            }
            ...
            return res.end(
              JSON.stringify({
                success: true,
                warning: "Settlement ready — provider configuration required.",
              }),
            )
          }
...
function parseRequestBody(req: any): Promise<string> {
