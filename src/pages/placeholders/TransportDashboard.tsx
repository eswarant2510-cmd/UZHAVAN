...
import { gpsTrackingManager, type GpsCoordinate } from "../../services/gpsTrackingService"
import { findBackhaulOpportunities } from "../../services/loadPoolingService"
          ...
          {selectedDocket && (
            <div className="md:col-span-7 space-y-6">
              <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-7 space-y-5">
                ...
                <RouteMap
                  origin={selectedDocket.pickupLocation}
                  destination={selectedDocket.deliveryLocation}
                  distanceKm={165}
                  etaFormatted="4 hrs"
                  transporterGps={currentGps ? { lat: currentGps.lat, lng: currentGps.lng } : undefined}
                />

                {/* GPS TRACKING CONTROLS FOR ACTIVE SHIPMENT */}
                <div className="bg-slate-900 text-white rounded-2xl p-4 flex flex-wrap justify-between items-center text-xs">
                  <div>
                    <p className="font-extrabold text-emerald-400 uppercase tracking-wider text-[10px]">
                      📡 Device GPS Tracking Stream
                    </p>
                    <p className="text-slate-300 mt-0.5">
                      {isGpsActive && currentGps
                        ? `Live GPS Pin: ${currentGps.lat.toFixed(4)}°, ${currentGps.lng.toFixed(4)}° (${currentGps.lastUpdated})`
                        : "Tracking inactive. Explicit device location permission required."}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      if (isGpsActive) {
                        gpsTrackingManager.stopTracking(selectedDocket.orderId)
                        setIsGpsActive(false)
                      } else {
                        const ok = gpsTrackingManager.requestConsentAndStartTracking(
                  ...
                  >
                    {isGpsActive ? "Pause GPS Stream" : "▶ Start Device GPS Tracking"}
                  </button>
                </div>
