...
import VirtualBillModal from "../../components/common/VirtualBillModal"
import { createPipeOrder, verifyPipePayment, PIPE_CONFIG } from "../../services/pipePaymentService"
...
export default function Orders() {
  ...
  const [pipeModalOrder, setPipeModalOrder] = useState<Order | null>(null)
  ...
  // PiPe Payment Handler
  async function handlePayWithPipe(order: Order) {
    setError(null)
    setProcessingOrderId(order.id)

    try {
      const res = await createPipeOrder({
        orderId: order.id,
        amount: order.amount,
        currency: "INR",
        buyerId: order.buyerPhone || "usr_136",
      })

      if (!res.success) {
        if (!PIPE_CONFIG.isConfigured) {
          // Open PiPe Payment Modal for Sandbox Verification
          setPipeModalOrder(order)
          setProcessingOrderId(null)
          return
        }
        throw new Error(res.error || "PiPe order creation failed.")
      }
      ...
      setError(err.message || "Failed to initiate PiPe payment.")
    } finally {
      setProcessingOrderId(null)
    }
  }
  ...
  async function handleSimulateSuccess(order: Order) {
    ...
    try {
      ...
      await farmerApi.savePaymentTransaction({
        orderId: order.id,
        provider: "razorpay",
    ...
    <div className="space-y-6">
              ...
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  ...
                  <div className="text-left sm:text-right shrink-0 space-y-1">
                    ...
                    <div>
                      <span
                        className={`text-[9px] font-black uppercase px-2.5 py-1 rounded inline-block border ${
                          isDisputed
                    ...
                    <button
                      ...
                      className="mt-1 px-3 py-1.5 bg-[#122b16] hover:bg-[#1c4021] text-white text-[11px] font-extrabold rounded-xl transition cursor-pointer flex items-center gap-1 shadow-xs"
                    >
                      <span>📄</span> View Virtual Bill
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-3">
                    <h3 className="text-xs font-extrabold uppercase text-slate-500 tracking-wider">
                      🛡️ Protected Escrow Properties (80/20 Settlement Model)
                    </h3>
                    <div className="text-[11px] space-y-1.5 font-semibold text-slate-600">
                      ...
                      {o.paymentStatus === "VERIFIED" && (
                        <div className="pt-2 border-t border-slate-200/60 space-y-1 text-[10.5px]">
                          <div className="flex justify-between text-emerald-800">
                            <span>⚡ Immediate 80% Farmer Release:</span>
                            <span className="font-black">₹{calculate8020Settlement(o.amount).immediateRelease.toLocaleString("en-IN")}</span>
                          </div>
                          <div className="flex justify-between text-amber-800">
                            <span>🔒 Retained 20% Settlement Hold:</span>
                            <span className="font-black">₹{calculate8020Settlement(o.amount).held.toLocaleString("en-IN")}</span>
                          </div>
                        </div>
                      )}
                      {isDisputed && (
                        <div className="bg-red-50 text-red-800 p-2.5 rounded-lg text-[10px] mt-1 space-y-0.5">
                          <p className="font-black">
                            ⚠️ Active Dispute: 20% Hold Retained
                          </p>
                    ...
                    {isPending && (
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={() => handlePayWithPipe(o)}
                          className="min-h-10 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black shadow-md hover:scale-[1.01] transition disabled:opacity-50 flex items-center gap-1.5 cursor-pointer"
                        >
                          <span>💳</span>
                          <span>{isProcessing ? "Processing PiPe..." : "Pay with PiPe"}</span>
                        </button>
                        <button
                          disabled={isProcessing}
                          onClick={() => handleStartPayment(o)}
                          className="min-h-10 px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-black shadow-xs transition disabled:opacity-50 cursor-pointer"
                        >
                          Alternative Gateway
                        </button>
                      </div>
      ...
      {pipeModalOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full border border-blue-200 shadow-2xl p-6 sm:p-7 space-y-5 relative">
            <button
              onClick={() => setPipeModalOrder(null)}
            ...
            <div className="text-center space-y-1.5 border-b border-slate-100 pb-4">
              <span className="inline-block bg-blue-100 text-blue-800 text-[10px] font-black uppercase px-3 py-1 rounded-full tracking-wider">
                💳 PIPE PAYMENT GATEWAY
              </span>
              <h2 className="text-xl font-black text-slate-900">UZHAVAN Payment</h2>
              <p className="text-xs text-slate-500 font-medium">
                Order #{pipeModalOrder.id} · Total: ₹{pipeModalOrder.amount.toLocaleString("en-IN")}
              </p>
            </div>

            {/* 80/20 Settlement Protection Summary */}
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50/50 border border-blue-100 p-4 rounded-2xl space-y-2 text-xs font-semibold">
              <span className="text-[10px] font-black text-blue-900 uppercase tracking-wider block">
                🛡️ Settlement Protection Model
              </span>
              <div className="flex justify-between text-emerald-800">
                <span>⚡ 80% Immediate Release:</span>
                <span className="font-black">₹{calculate8020Settlement(pipeModalOrder.amount).immediateRelease.toLocaleString("en-IN")}</span>
              </div>
              <div className="flex justify-between text-amber-800">
                <span>🔒 20% Retained Hold:</span>
                <span className="font-black">₹{calculate8020Settlement(pipeModalOrder.amount).held.toLocaleString("en-IN")}</span>
              </div>
              <p className="text-[9.5px] text-slate-500 pt-1 font-medium italic border-t border-blue-100/60">
                The 20% hold remains protected until delivery verification is confirmed by logistics.
              </p>
            </div>

            {!PIPE_CONFIG.isConfigured && (
              <div className="bg-amber-50 text-amber-900 border border-amber-200 text-[10.5px] p-3 rounded-2xl font-bold leading-relaxed">
                ⚠️ Sandbox Verification Mode: VITE_PIPE_API_KEY environment variable is not configured. Proceeding with sandbox verification.
              </div>
            )}
            ...
            <div className="flex gap-3 pt-1">
              <button
                onClick={() => setPipeModalOrder(null)}
                className="flex-1 py-3 border border-slate-200 rounded-xl text-xs font-bold hover:bg-slate-50 text-slate-600 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={async () => {
                  const targetOrder = pipeModalOrder
                  setPipeModalOrder(null)
                  setProcessingOrderId(targetOrder.id)
                  try {
                    const mockPipeOrderId = `pipe_ord_${Math.random().toString(36).slice(2, 10)}`
                    const mockPipePayId = `pipe_pay_${Math.random().toString(36).slice(2, 12)}`

                    await farmerApi.savePaymentTransaction({
                      orderId: targetOrder.id,
                      provider: "PiPe Gateway",
                      providerOrderId: mockPipeOrderId,
                      providerPaymentId: mockPipePayId,
                      amount: targetOrder.amount,
                      status: "captured",
                    })
                    await farmerApi.updateOrderStatus(targetOrder.id, "PAID")
                    await loadOrders()
                  } catch (err: any) {
                    setError(err.message || "PiPe payment verification failed.")
                  } finally {
                    setProcessingOrderId(null)
                  }
                }}
                className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black shadow-md transition cursor-pointer"
              >
                Complete PiPe Payment
              </button>
            </div>
          </div>
        </div>
