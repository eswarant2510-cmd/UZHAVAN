...
export default function LotDetail() {
  ...
  const lotOwner = lot?.createdByLoginNumber || lot?.ownerId || lot?.farmerPhone || "588"
  const isOwner = Boolean(lot && (lotOwner === activeLogin))
  ...
  async function handleSaveChanges(e: React.FormEvent) {
    ...
    if (!isOwner) {
      setError(`Unauthorized: You cannot edit a lot created by Farmer #${lotOwner}.`)
      return
    }
  ...
  async function handleCancelLot() {
    if (!isOwner) {
      setError(`Unauthorized: You cannot cancel a lot created by Farmer #${lotOwner}.`)
      return
    }
    if (
      !window.confirm(
  ...
  async function handleDeleteLot() {
    if (!isOwner) {
      setError(`Unauthorized: You cannot delete a lot created by Farmer #${lotOwner}.`)
      return
    }
    if (!window.confirm("Are you sure you want to delete this smart lot permanently?")) {
      return
    }
    setError(null)
    setCancelling(true)
            ...
            <div className="bg-white border border-[#a7e4b0]/70 rounded-3xl overflow-hidden shadow-sm">
              <div className="h-64 sm:h-80 bg-slate-100 relative">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-xs font-extrabold px-3 py-1.5 rounded-full shadow border ${
                      isOwner
                        ? "bg-emerald-100 text-emerald-800 border-emerald-300"
                        : "bg-amber-100 text-amber-900 border-amber-300"
                    }`}
                  >
                    {isOwner ? `👑 Owner (Farmer #${activeLogin})` : `🔒 View Only (Owner: #${lotOwner})`}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 sm:p-8 space-y-6">
                <div>
                  ...
                  <p className="text-slate-400 text-xs mt-1">
                    Storage Location: {lot.location} · Created by Farmer #{lotOwner}
                  </p>
                </div>
                ...
                {isOwner ? (
                  lot.status !== "cancelled" && (
                    <div className="flex flex-wrap sm:flex-nowrap gap-3 pt-4 border-t border-slate-100">
                      <button
                        onClick={handleCancelLot}
                        disabled={cancelling}
                        className="flex-1 py-3 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 font-bold rounded-2xl text-xs transition cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        disabled={cancelling}
                        className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-2xl text-xs transition cursor-pointer"
                      >
                        🗑️ Delete Lot
                      </button>
                    </div>
                  ...
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 font-semibold space-y-1">
                    <p className="font-extrabold text-sm flex items-center gap-1.5">
                      <span>🔒</span> View Only Mode
                    </p>
                    <p>
                      This lot belongs to Farmer #{lotOwner}. Editing, deleting, and cancelling options are restricted to the lot owner.
                    </p>
                  </div>
                )}
              </div>
                ...
                >
                  <div className="space-y-2">
                    ...
                    <div className="pt-0.5">
                      <TrustScoreCard userPhone="9876500001" role="buyer" compact />
                    </div>
