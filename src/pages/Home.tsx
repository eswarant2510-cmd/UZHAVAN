...
function RoleCard({ role }: { role: typeof ROLES[0] }) {
  ...
  const handleRoleSelection = (e: React.MouseEvent) => {
    ...
    if (role.id === "transport" || role.id === "admin") {
      navigate(role.path)
    } else {
    ...
    >
      <div className="relative flex flex-col min-h-screen">
        ...
        <header className="relative z-10 px-6 py-5 flex items-center justify-between max-w-6xl mx-auto w-full">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <button
              ...
              className="ml-2 px-4 py-2 text-sm font-semibold rounded-lg text-green-900 hover:scale-105 transition-transform cursor-pointer"
              style={{
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-16 text-center max-w-4xl mx-auto w-full">
          ...
          <p
            className="text-sm font-medium mb-8 tracking-wider"
          ...
          {/* Voice Assistant 4-Language Quick Selector Widget */}
          <div className="mb-10 w-full max-w-xl mx-auto rounded-2xl p-4 border border-green-500/40 bg-black/50 backdrop-blur-md shadow-2xl text-left">
            <div className="flex items-center justify-between gap-3 mb-3">
              <div className="flex items-center gap-2 text-white font-black text-xs tracking-wider uppercase">
                <span className="text-base animate-pulse">🎙️</span>
                <span>UZHAVAN VOICE ASSISTANT — 4 LANGUAGES SUPPORTED</span>
              </div>
              <button
                type="button"
                onClick={() => window.dispatchEvent(new Event("open-uzhavan-voice"))}
                className="px-3.5 py-1.5 rounded-xl text-xs font-black text-white bg-[#2e7d3a] hover:bg-[#3da64e] transition cursor-pointer shadow-md"
              >
                Open Voice AI ➔
              </button>
            </div>
            <div className="flex items-center gap-2 flex-wrap text-xs">
              <span className="text-slate-300 font-semibold mr-1">Choose Language:</span>
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() => {
                    localStorage.setItem("uzhavan_voice_lang", lang.id)
                    window.dispatchEvent(new Event("open-uzhavan-voice"))
                  }}
                  className="px-3 py-1.5 rounded-xl border border-white/20 bg-white/10 text-white font-bold hover:bg-green-600 hover:border-green-400 transition cursor-pointer"
                >
                  {lang.label}
                </button>
              ))}
