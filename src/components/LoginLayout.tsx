...
export default function LoginLayout({
  ...
  let defaultPhone = userRole === "buyer" ? "136" : "9876500001"
  ...
  async function handleLogin(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    const cleanInput = phone.trim() || defaultPhone

    setLoading(true)
    try {
      const { error: authError } = await supabaseSignInWithMockOtp(cleanInput, userRole)
      if (authError) {
      ...
      setLoading(false)
      navigate(userRole === "farmer" ? "/farmer" : `/dashboard/${userRole}`)
    } catch (err: any) {
              ...
              <form onSubmit={handleLogin} className="space-y-4" noValidate>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder={userRole === "buyer" ? "Enter Buyer ID (136, 163, 552)" : "e.g. 9876500001"}
                      value={phone}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button
                          key={b.id}
                          type="button"
                          onClick={() => setPhone(b.id)}
                          className={`px-3 py-2 rounded-xl text-xs font-bold transition flex flex-col items-center justify-center border cursor-pointer ${
                            phone.trim() === b.id
                              ? "bg-blue-600 text-white border-blue-600 shadow-sm"
                              : "bg-slate-50 text-slate-700 border-slate-200 hover:bg-blue-50"
                          }`}
                        >
                          <span>Buyer #{b.id}</span>
                          <span className="text-[10px] opacity-80">{b.name}</span>
                        </button>
                      ))}
                    </div>
                  </div>
