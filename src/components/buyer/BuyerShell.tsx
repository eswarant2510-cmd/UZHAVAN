...
const NAV = [
  { to: "/dashboard/buyer", label: "Home", icon: HomeIcon, end: true },

  { to: "/dashboard/buyer/discover", label: "Discover", icon: MarketIcon },

  { to: "/dashboard/buyer/match", label: "Smart Match", icon: DecisionIcon },

  { to: "/dashboard/buyer/orders", label: "Orders", icon: OrdersIcon },

  { to: "/dashboard/buyer/payments", label: "Payments", icon: EarningsIcon },

  { to: "/dashboard/buyer/transport", label: "Transport", icon: TruckIcon },

  { to: "/dashboard/buyer/notifications", label: "Alerts", icon: BellIcon },

  { to: "/dashboard/buyer/profile", label: "Profile", icon: ProfileIcon },
]
      ...
      try {
        ...
        if (profile) {
          if (profile.role === "buyer") {
            ...
            // Strict role security: redirect farmers away from buyer portal
            navigate("/farmer")
          } else {
    ...
    <div className="min-h-screen flex" style={{ background: "#f3faf4" }}>
      ...
      >
        ...
        <div className="p-4 border-t border-white/10">
          <button
            onClick={() => navigate("/dashboard/buyer/discover")}
          ...
          <div className="flex items-center justify-between text-xs text-white/50">
            <span className="truncate pr-2">{user?.name ?? "Buyer"}</span>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="sticky top-0 z-20 flex items-center justify-between px-4 py-3 bg-white/90 backdrop-blur border-b border-blue-100">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setOpen(true)}
              className="lg:hidden p-2 rounded-xl bg-blue-50 text-blue-800"
          ...
          <button
            type="button"
            onClick={() => window.dispatchEvent(new Event("open-uzhavan-voice"))}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-50 border border-blue-200 text-xs font-black text-blue-700 hover:bg-blue-100 transition cursor-pointer shadow-sm"
          >
        ...
        <VoiceAssistant role="buyer" currentLotId={lotId} />
      </div>
    </div>
  )
}
