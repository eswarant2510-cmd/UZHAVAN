...
export const farmerApi = {
  ...
  async getDashboard(): Promise<FarmerDashboardData> {
    ...
    try {
      ...
      // 1. Fetch lots matching authenticated owner_id (UUID) or legacy farmer_phone
      const userAuthId = user.userId || user.id
      const userPhone = user.phone

      let query = supabase.from("smart_lots").select("*")
      if (userAuthId && userPhone) {
        query = query.or(`owner_id.eq.${userAuthId},farmer_phone.eq.${userPhone}`)
      } else if (userAuthId) {
        query = query.eq("owner_id", userAuthId)
      } else {
        query = query.eq("farmer_phone", userPhone)
      }
  ...
  async getLots(): Promise<SmartLot[]> {
    try {
      ...
      const query = supabase.from("smart_lots").select("*")
      ...
      return (data || []).map((row: any) => {
        ...
        return {
          id: row.id,
          createdByLoginNumber: row.created_by_login_number || row.owner_id || row.farmer_phone || "588",
          ownerId: row.owner_id || row.created_by_login_number || row.farmer_phone || "588",
  ...
  async getLot(id: string): Promise<SmartLot | null> {
    try {
      ...
      return {
        id: data.id,
        createdByLoginNumber: data.created_by_login_number || data.owner_id || data.farmer_phone || "588",
        ownerId: data.owner_id || data.created_by_login_number || data.farmer_phone || "588",
      ...
      if (error) {
        if (error.code === "42703" || error.message?.includes("column")) {
          const fallbackPayload = {
            id: lot.id,
            farmer_phone: phone,
            created_by_login_number: phone,
            owner_id: authUserId,
            crop: lot.variety ? `${lot.crop} (${lot.variety})` : lot.crop,
            quantity_kg: lot.quantityKg,
            image_url: lot.imageUrl || (lot.photos && lot.photos.join(",")) || "",
            location: lot.location,
            status: lot.status || "active",
            quality_grade: "A",
            quality_confidence: 88,
  ...
  async updateLot(
    ...
    const user = (await getAuthProfile()) || readSession() || DEMO_FARMER
    const activeLogin = user.loginNumber || user.phone || "588"

    const existingLot = await farmerApi.getLot(id)
    if (existingLot) {
      const owner = existingLot.createdByLoginNumber || existingLot.ownerId || existingLot.farmerPhone
      if (owner && owner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot edit lot created by Farmer #${owner}`)
      }
    }
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update(fullPayload)
        .eq("id", id)
        .eq("created_by_login_number", activeLogin)
    ...
    const mockIdx = MOCK_LOTS.findIndex((l) => l.id === id)
    if (mockIdx !== -1) {
      const mockOwner = MOCK_LOTS[mockIdx].createdByLoginNumber || MOCK_LOTS[mockIdx].ownerId || MOCK_LOTS[mockIdx].farmerPhone
      if (mockOwner && mockOwner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot edit lot created by Farmer #${mockOwner}`)
      }
      MOCK_LOTS[mockIdx] = ({
        ...MOCK_LOTS[mockIdx],
        ...lot,
        photos: lot.photos || MOCK_LOTS[mockIdx].photos,
      } as SmartLot)
    }
  },
  ...
  async cancelLot(id: string): Promise<void> {
    ...
    if (existingLot) {
      const owner = existingLot.createdByLoginNumber || existingLot.ownerId || existingLot.farmerPhone
      if (owner && owner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot cancel lot created by Farmer #${owner}`)
      }
    }
    ...
    if (mockIdx !== -1) {
      const mockOwner = MOCK_LOTS[mockIdx].createdByLoginNumber || MOCK_LOTS[mockIdx].ownerId || MOCK_LOTS[mockIdx].farmerPhone
      if (mockOwner && mockOwner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot cancel lot created by Farmer #${mockOwner}`)
      }
      MOCK_LOTS[mockIdx].status = "cancelled"
    }
  },
  ...
  async deleteLot(id: string): Promise<void> {
    ...
    if (existingLot) {
      const owner = existingLot.createdByLoginNumber || existingLot.ownerId || existingLot.farmerPhone
      if (owner && owner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot delete lot created by Farmer #${owner}`)
      }
    }
    ...
    if (mockIdx !== -1) {
      const mockOwner = MOCK_LOTS[mockIdx].createdByLoginNumber || MOCK_LOTS[mockIdx].ownerId || MOCK_LOTS[mockIdx].farmerPhone
      if (mockOwner && mockOwner !== activeLogin) {
        throw new Error(`Unauthorized: Farmer #${activeLogin} cannot delete lot created by Farmer #${mockOwner}`)
      }
      MOCK_LOTS.splice(mockIdx, 1)
    }
  },
  ...
  async getPaymentSettlement(orderId: string): Promise<PaymentSettlement | null> {
    try {
      if (isSupabaseConfigured()) {
        const { data, error } = await supabase
        ...
        if (data) {
          return {
            ...
            authoritativeAmount: Number(data.authoritative_amount),
            immediateReleaseAmount: Number(data.immediate_release_amount),
            heldAmount: Number(data.held_amount),
            remainingReleaseAmount: Number(data.remaining_release_amount),
            status: data.status,
      ...
      const raw = localStorage.getItem(`settlement_${orderId}`)
      return raw ? JSON.parse(raw) : null
    } catch {
      const raw = localStorage.getItem(`settlement_${orderId}`)
      return raw ? JSON.parse(raw) : null
  ...
  async savePaymentTransaction(txn: any): Promise<void> {
    try {
      // 1. Fetch authoritative order to get true amount and ownership UUIDs
      const order = await this.getOrder(txn.orderId)
      if (!order) throw new Error("Order not found for payment transaction")

      // Authoritative amount comes directly from trusted order entity
      const authoritativeAmount = order.amount
      const { immediateRelease, held } = calculate8020Settlement(authoritativeAmount)

      let transactionRecordId: string | undefined = undefined
      ...
      if (isSupabaseConfigured()) {
        ...
        if (existing) {
          ...
          const { data: inserted, error } = await supabase.from("payment_transactions").insert({
          ...
          if (error) throw error
          if (inserted) transactionRecordId = inserted.id
        }

        // Create or update 80/20 payment_settlements ledger record
        if (txn.status === "captured" || txn.status === "VERIFIED") {
          const { error: setErr } = await supabase.from("payment_settlements").upsert({
            order_id: txn.orderId,
            payment_transaction_id: transactionRecordId,
            authoritative_amount: authoritativeAmount,
            immediate_release_amount: immediateRelease,
            held_amount: held,
            remaining_release_amount: 0,
            status: "HELD",
            farmer_id: order.farmerUserId,
            buyer_id: order.buyerUserId,
          }, { onConflict: "order_id" })
          if (setErr) console.warn("Supabase payment_settlements upsert note:", setErr)
        }
      }

      const settlementRecord: PaymentSettlement = {
        orderId: txn.orderId,
        paymentTransactionId: transactionRecordId,
        authoritativeAmount,
      ...
      localStorage.setItem(`txn_${txn.orderId}`, JSON.stringify({ ...txn, amount: authoritativeAmount }))
      localStorage.setItem(`settlement_${txn.orderId}`, JSON.stringify(settlementRecord))
    } catch (err) {
      console.warn("savePaymentTransaction fallback execution:", err)
      const order = await this.getOrder(txn.orderId)
      const authoritativeAmount = order ? order.amount : txn.amount
      const { immediateRelease, held } = calculate8020Settlement(authoritativeAmount)
  ...
  async settleOrderFunds(orderId: string): Promise<void> {
    ...
    // Update 80/20 payment_settlements ledger to FULLY_SETTLED
    const settlement = await this.getPaymentSettlement(orderId)
    if (settlement) {
      const updatedSettlement: PaymentSettlement = {
        ...settlement,
        remainingReleaseAmount: settlement.heldAmount,
      ...
      try {
        if (isSupabaseConfigured()) {
          await supabase
            .from("payment_settlements")
            .update({
              remaining_release_amount: settlement.heldAmount,
              status: "FULLY_SETTLED",
              updated_at: new Date().toISOString(),
            })
            .eq("order_id", orderId)
      ...
      localStorage.setItem(`settlement_${orderId}`, JSON.stringify(updatedSettlement))
    }
  },
